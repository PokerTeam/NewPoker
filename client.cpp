...
Client::Client(int desc,Server *serv)
{
    ...
    connect(this,SIGNAL(Login(QString,QString)),this,SLOT(doLogin(QString ,QString )));
    connect(this,SIGNAL(Registration(QString,QString)),this,SLOT(doRegistration(QString,QString)));
    connect(this,SIGNAL(SendCommandToCurrentUser(quint8,QString)),this,SLOT(doSendCommand(quint8,QString)));
    connect(this,SIGNAL(SendCommandToAll(quint8,QString)),this,SLOT(doSendCommandAll(quint8,QString)));
}
...
void Client::onReadyRead()
{
    QDataStream in(_sok);
       ...
       if (!_isAutched && command != Commands::comAutchReq && command != Commands::comRegAcc)
           return;
       switch(command)
           {
               ...
               case Commands::comAutchReq:
               {
                  ...
                  {
                      _serv->outToConsole("connect "+login+" with ip address "+this->_sok->localAddress().toString(),10);
                      _isAutched = true;
                  ...
                  {
                       _serv->outToConsole("Don't connect "+login+" with ip address "+this->_sok->localAddress().toString(),12);
               ...
               case Commands::comRegAcc:

                {
                 ...
                 pass = buf.split(";")[1];
                  ...
                  {
                  _serv->outToConsole("Register successful Login: "+login+" Password: "+pass,10);
...
void Client::onError(QAbstractSocket::SocketError socketError) const
{
    ...
    case QAbstractSocket::ConnectionRefusedError:
        ...
        break;
    default:
        _serv -> outToConsole("Error - The following error occurred: "+_sok->errorString(),12);
    }
